...
public class TiltShiftFilter: CIFilter {
    ...
    private func ciImage(from filterName: String, parameters: [String: Any]) -> CIImage? {
        guard let filtered = CIFilter(name: filterName, parameters: parameters) else { return nil }
    ...
    override public var outputImage: CIImage? {
        ...
        guard let gradientImage = ciImage(from: "CILinearGradient", parameters: gradientParameters) else { return nil }
        
        gradientParameters["inputPoint0"] = CIVector(x: 0, y: 0.25 * inputImage.extent.height)
        
        guard let backgroundGradientImage = ciImage(from: "CILinearGradient", parameters: gradientParameters) else { return nil }
        ...
        guard let maskImage = ciImage(from: "CIAdditionCompositing", parameters: maskParameters) else { return nil }
    ...
    convenience init?(image: UIImage, radius: Double = TiltShiftFilter.defaultRadius) {
        guard let backing = image.ciImage ?? CIImage(image: image) else { return nil }
